...
- [kernel/__init__.py](file://vivify/kernel/__init__.py)
</cite>

## 更新摘要
**所做更改**
- 更新升级标题和消息的品牌一致性，从 "auto-heal" 切换到 "vivify"
- 增强部署条件逻辑，确保仅在 PR 实际合并后执行部署
- 完善自动目标分解功能的文档说明
- 更新配置系统中环境变量前缀的说明
- 增强部署器初始化和执行流程的文档
- **新增** 从特征管道中移除冗余数据类的架构简化，提升代码组织和维护性
...
**新增** 本次更新还反映了特征管道架构的重大简化，通过从 FeatureRunReport 数据类中移除冗余的 @dataclass 装饰器，提升了代码组织和维护性。这一变化体现了项目对代码简洁性和性能优化的关注。
...
- **特征管道 (Feature Pipeline)**: 端到端的特性开发、验证和跟进流程
...
**更新** Vivify 项目最近完成了特征管道架构的重要简化，移除了冗余的数据类装饰器，提升了代码组织和维护性。

#### FeatureRunReport 类重构

在特征管道中，`FeatureRunReport` 类经历了重要的架构简化：

**重构前**: 使用 `@dataclass` 装饰器的完整数据类定义
**重构后**: 普通类定义，移除了不必要的数据类装饰器

这种重构带来了以下优势：
- **代码简洁性**: 移除了冗余的装饰器语法
- **性能优化**: 减少了数据类生成的元数据开销
- **维护性提升**: 简化的类结构更易于理解和修改
- **向后兼容**: 保持相同的 API 接口和功能

#### 架构简化的影响

```mermaid
graph LR
A[FeatureRunReport 数据类] --> B[普通类定义]
B --> C[移除 @dataclass 装饰器]
C --> D[保持相同属性]
D --> E[简化初始化逻辑]
E --> F[提升代码可读性]
```
...
**图表来源**
- [feature_pipeline.py:67-76](file://vivify/kernel/feature_pipeline.py#L67-L76)

#### 重构后的类结构

重构后的 `FeatureRunReport` 类保持了原有的属性定义，但采用了更简洁的实现方式：

- `feature_id`: int - 特性 ID
- `status`: str - 当前状态，默认 "pending"
- `pr`: Optional[PullRequest] - 关联的 PR 对象
- `quality`: Optional[QualityCheckResult] - 质量检查结果
- `followups_created`: int - 创建的跟进特性数量
- `error`: Optional[str] - 错误信息
- `durations`: dict - 各阶段耗时统计

**章节来源**
- [feature_pipeline.py:67-76](file://vivify/kernel/feature_pipeline.py#L67-L76)
- [kernel/__init__.py:12-16](file://vivify/kernel/__init__.py#L12-L16)

### 特征状态机集成

特征管道继续与状态机系统紧密集成，确保状态转换的合法性和一致性：
...
participant Storage as 存储系统
Pipeline->>FSM : validate_transition(current, target)
FSM-->>Pipeline : 验证结果
alt 合法转换
Pipeline->>Storage : 更新特性状态
else 非法转换
Pipeline->>Pipeline : 记录警告并跳过
end
```

**图表来源**
- [feature_states.py:50-64](file://vivify/kernel/feature_states.py#L50-L64)

#### 状态机功能

状态机系统提供了以下核心功能：
- **转换验证**: 检查状态转换是否符合预定义规则
- **终端状态检测**: 识别是否到达终态
- **活跃状态识别**: 判断特性是否仍需处理
- **阶段映射**: 将状态映射到处理阶段

**章节来源**
- [feature_states.py:14-28](file://vivify/kernel/feature_states.py#L14-L28)
- [feature_states.py:50-79](file://vivify/kernel/feature_states.py#L50-L79)
...
U[vivify.kernel.feature_states]
V[vivify.kernel.__init__]
end
subgraph "配置文件"
W[.vivify.example.yml]
X[pyproject.toml]
Y[GOALS.md.tmpl]
end
A --> H
...
**新增** 特征管道的架构简化带来了显著的性能提升：

#### 数据类移除的性能收益

- **内存占用减少**: 移除数据类装饰器消除了元数据存储开销
- **初始化速度提升**: 普通类的构造函数比数据类更快
- **序列化性能改善**: 简化的类结构减少了 JSON 序列化复杂度
- **调试友好性**: 更直观的类结构便于性能分析和调试

#### 状态机集成的性能优化

- **状态验证开销**: 状态机验证在每次状态转换时进行，开销很小
- **转换表查找**: 使用字典查找实现 O(1) 状态转换验证
- **终端状态快速检测**: 直接集合查找判断终态，效率高
...
7. **数据类重构**: 继续审视其他数据类的必要性，移除冗余装饰器
8. **状态机优化**: 考虑将状态转换表缓存到内存中
...
**FeatureRunReport 类导入失败**:
- 检查 `kernel/__init__.py` 中的导出声明
- 验证 `feature_pipeline.py` 中的类定义
- 确认数据类装饰器移除后的兼容性
...
10. **特征管道测试**:
    ...
    from vivify.config.schema import FeaturePipelineConfig
    print('FeaturePipeline import OK')
    print('FeatureRunReport import OK')
    print('FeaturePipelineConfig import OK')
    "
    ```
11. **状态机测试**:
    ```bash
    python3 -c "
    from vivify.kernel.feature_states import FeatureStateMachine
    print('FeatureStateMachine import OK')
    print('can_transition:', FeatureStateMachine.can_transition('pending', 'evaluating'))
    "
    ```
...
**新增** 本次更新还反映了特征管道架构的重大简化，通过从 FeatureRunReport 数据类中移除冗余的 @dataclass 装饰器，提升了代码组织和维护性。这一变化体现了项目对代码简洁性和性能优化的关注，同时保持了完整的功能兼容性。

自动部署功能采用插件化架构，支持 SSH、命令和 Webhook 三种部署方式，为用户提供了灵活的部署选择。新增的日志记录机制提供了清晰的部署状态反馈，包括自动合并超时和手动合并场景的详细信息。

这次改造不仅更新了代码中的品牌标识，还涉及配置文件、文档、测试等多个方面，体现了完整的软件工程实践。通过详细的文档记录和验证流程，确保了改造过程的可控性和可追溯性。自动目标分解功能、增强的部署条件逻辑、特征管道架构简化和自动部署功能的集成进一步增强了 Vivify 的自动化能力，为用户提供从问题检测、目标分解到代码部署的完整解决方案。