...
## 更新摘要
**所做更改**
- 新增自动目标分解功能章节，详细说明_goals_file_hash和_last_decompose_time状态管理
- 添加_goals_auto_decomposition阶段的完整实现说明
- 更新内核循环架构图，增加目标分解器集成节点
- 添加GoalsConfig配置集成和CLI命令支持
- 完善文档验证和一致性检查方法
...
**更新** 本次更新还完整实现了内核中集成的自动目标分解功能和自动部署功能。自动目标分解功能通过_goals_file_hash和_last_decompose_time状态管理、_maybe_decompose_goals()方法实现、以及GoalsConfig配置集成，为用户提供了从项目目标到可执行特性的完整自动化流程。自动部署功能采用插件化架构，支持 SSH、命令和 Webhook 三种部署方式，为用户提供了灵活的部署选择。

这次改造不仅更新了代码中的品牌标识，还涉及配置文件、文档、测试等多个方面，体现了完整的软件工程实践。通过详细的文档记录和验证流程，确保了改造过程的可控性和可追溯性。自动目标分解功能和自动部署功能的集成进一步增强了 Vivify 的自动化能力，为用户提供从问题检测、目标分解到代码部署的完整解决方案。