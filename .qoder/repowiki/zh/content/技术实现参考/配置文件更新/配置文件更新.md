...
W --> AA[PR 模式集成]
AA --> AB[AutoMerge 类]
AB --> AC[_poll_until_merged 方法]
AC --> AD[超时计算逻辑]
end
subgraph "配置组件"
W --> AE[CLI 集成]
AE --> AF[run_cmd.py]
AF --> AG[AutoMergeConfig 构造]
AG --> AH[poll_timeout_seconds 参数]
end
```
...
- [vivify/cli/run_cmd.py: 124-128:124-128](file://vivify/cli/run_cmd.py#L124-L128)
...
- [vivify/config/schema.py: 18-26:18-26](file://vivify/config/schema.py#L18-L26)
...
- [vivify/pr_mode/auto_merge.py: 29-36:29-36](file://vivify/pr_mode/auto_merge.py#L29-L36)
...
subgraph "前端组件"
O[index.html] --> P[Chart.js 图表]
Q[app.js] --> R[WebSocket 连接]
S[样式文件] --> T[响应式设计]
end
B --> O
B --> Q
...
8. **配置覆盖验证**：**新增** 验证环境变量对 merge_poll_timeout_seconds 的覆盖效果
...
这个配置系统为Vivify项目提供了坚实的基础，支持未来的功能扩展和维护需求，特别是在自动化部署和GitHub集成方面的强大能力为项目的持续交付提供了重要保障。实例级GitHub令牌配置的引入进一步增强了配置系统的灵活性和安全性，为不同环境下的部署需求提供了更好的支持。新增的 PR 合并轮询控制功能为项目的自动化流程提供了更精细的控制能力，确保在不同项目规模和CI环境下都能获得最佳的用户体验。