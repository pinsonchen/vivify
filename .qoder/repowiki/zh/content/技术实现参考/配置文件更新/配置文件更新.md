...
## 更新摘要
**所做更改**
- 新增 DeployConfig 类，提供完整的部署配置支持
- 添加 SSH 部署器，支持 rsync 和 git_pull 模式
- 新增自定义命令部署器，支持任意 shell 命令部署
- 新增 Webhook 部署器，支持外部服务触发部署
- 更新配置模式，包含 deploy 配置块
- 新增部署器工厂函数和部署结果数据类
- 更新项目配置，包含部署相关信息
...
subgraph "配置组件"
S[Schema 定义] --> T[Pydantic 模型]
U[Loader] --> V[配置合并]
W[Defaults] --> X[默认值管理]
Y[Templates] --> Z[模板渲染]
AA[Dashboard] --> BB[FastAPI 应用]
CC[Deployers] --> DD[部署器抽象]
EE[Kernel] --> FF[部署集成]
end
F --> T
B --> V
...
**图表来源**
- [vivify/config/schema.py: 151-219:151-219](file://vivify/config/schema.py#L151-L219)
...
**章节来源**
- [vivify/cli/init_cmd.py: 1-456:1-456](file://vivify/cli/init_cmd.py#L1-L456)
...
这个配置系统为Vivify项目提供了坚实的基础，支持未来的功能扩展和维护需求，特别是在自动化部署方面的强大能力为项目的持续交付提供了重要保障。