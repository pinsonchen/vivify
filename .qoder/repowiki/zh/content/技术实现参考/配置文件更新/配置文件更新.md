...
## 更新摘要
**所做更改**
- 新增 merge_poll_timeout_seconds 配置参数，控制 PR 合并轮询超时时间，默认值为 120 秒
- 更新 PR 配置模式，添加 merge_poll_timeout_seconds 字段定义
- 改进 AutoMerge 配置类，支持 merge_poll_timeout_seconds 参数
- 更新配置加载器，确保 merge_poll_timeout_seconds 正确加载和处理
- 更新 CLI 运行命令，将 merge_poll_timeout_seconds 传递给 AutoMerge
- 更新配置示例和模板文件，包含 merge_poll_timeout_seconds 参数
- 增强 PR 模式单元测试，验证 merge_poll_timeout_seconds 的功能
- **新增** 远程执行参数配置，支持 QoderCliConfig 中的远程执行参数
- **新增** 远程执行配置参数，包括 use_remote、remote_poll_interval、remote_timeout、max_concurrent_remote、plan_agent_for_decompose
...
- **PR 合并轮询控制**：**新增** 支持精确控制 PR 合并等待超时时间
- **远程执行支持**：**新增** 支持通过云远程会话执行AI任务
...
W --> AG[PR 模式集成]
AG --> AH[AutoMerge 类]
AH --> AI[_poll_until_merged 方法]
AI --> AJ[超时计算逻辑]
end
subgraph "配置组件"
W --> AK[CLI 集成]
AK --> AL[run_cmd.py]
AL --> AM[AutoMergeConfig 构造]
AM --> AN[poll_timeout_seconds 参数]
end
subgraph "远程执行组件"
AA --> AO[QoderCliAgent]
AO --> AP[RemoteSessionManager]
AP --> AQ[并发远程会话]
AQ --> AR[超时控制]
end
```
...
subgraph "前端组件"
S[index.html] --> T[Chart.js 图表]
U[app.js] --> V[WebSocket 连接]
W[样式文件] --> X[响应式设计]
end
B --> S
B --> U
...
9. **配置覆盖验证**：**新增** 验证环境变量对 merge_poll_timeout_seconds 和远程执行参数的覆盖效果
...
这个配置系统为Vivify项目提供了坚实的基础，支持未来的功能扩展和维护需求，特别是在自动化部署、GitHub集成和远程执行方面的强大能力为项目的持续交付和智能化执行提供了重要保障。实例级GitHub令牌配置的引入进一步增强了配置系统的灵活性和安全性，为不同环境下的部署需求提供了更好的支持。新增的 PR 合并轮询控制功能和远程执行支持为项目的自动化流程提供了更精细的控制能力和更强的执行能力，确保在不同项目规模和CI环境下都能获得最佳的用户体验。