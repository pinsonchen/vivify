...
## 更新摘要
**变更内容**
- CLI目标分解功能增强：支持在从目标创建特性请求时指定验证方法
- 仪表板验证跟踪一致性：保持与仪表板验证功能的验证方法要求一致
- 数据库迁移支持：verification_method字段的完整数据库支持
- 模板系统增强：目标分解模板强制要求verification_method字段
- 存储层集成：在特性请求创建时正确处理验证方法字段
...
**新增的CLI目标分解功能增强**使得用户能够直接通过命令行界面指定验证方法，确保从目标创建的特性请求具备明确的验证标准。这一功能与仪表板验证跟踪功能保持一致，确保系统在各个层面都遵循相同的验证方法要求。
...
**新增的CLI目标分解功能增强**是本次更新的核心内容，它允许用户在从目标创建特性请求时直接指定验证方法，确保生成的特性请求具备明确的验证标准。
...
```mermaid
flowchart TD
A[vivify goals decompose] --> B[解析GOALS.md]
B --> C[加载Agent配置]
C --> D[初始化目标分解器]
D --> E[收集上下文信息]
E --> F[调用AI代理]
...
if not args.dry_run:
    storage.create_feature(FeatureRequest(
        ...
        verification_method=s.verification_method,  # ← 新增：直接使用AI提供的验证方法
    ))
    total_created += 1
```
...
1. **模板一致性**：目标分解模板和验证模板都要求明确的验证方法
2. **字段一致性**：verification_method字段在所有相关组件中保持一致
3. **验证标准一致性**：相同的验证方法要求和示例指导
4. **存储一致性**：数据库中verification_method字段的统一处理
...
| 选项 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `--goal` | 字符串 | None | 限制处理特定目标名称 |
| `--dry-run` | 布尔值 | False | 仅打印提议的FeatureSpecs而不持久化 |
| `--json` | 布尔值 | False | 以JSON格式显示目标内容 |

**章节来源**
- [goals_cmd.py:109-166](file://vivify/cli/goals_cmd.py#L109-L166)
- [goal_decompose.md.j2:92-117](file://vivify/agents/prompts/templates/goal_decompose.md.j2#L92-L117)
- [feature_verify.md.j2:22-33](file://vivify/agents/prompts/templates/feature_verify.md.j2#L22-L33)
...
**问题15：CLI目标分解失败**
- **症状**：vivify goals decompose命令执行失败
- **原因**：GOALS.md文件不存在或配置错误
- **解决方案**：检查GOALS.md文件路径和配置文件，确保文件存在且格式正确

**问题16：验证方法存储失败**
- **症状**：特性请求创建成功但验证方法丢失
- **原因**：存储层未正确处理verification_method字段
- **解决方案**：检查SQLite提供者的create_feature方法，确保verification_method字段正确传递
...
11. **CLI集成增强**：支持从目标直接创建带验证方法的特性请求，提升用户体验
12. **仪表板一致性**：验证方法要求与仪表板验证功能保持一致，确保系统整体性

**新增的CLI目标分解功能增强**使得用户能够直接通过命令行界面指定验证方法：
- 支持在从目标创建特性请求时直接指定verification_method
- 保持与仪表板验证跟踪功能的验证方法要求一致
- 提供更直观的CLI交互体验
- 确保从目标到特性的完整验证方法传递
...
该功能为Vivify项目提供了强大的自动化能力，能够持续地将项目目标转化为具体的开发行动，提高团队的工作效率和项目交付质量。通过合理的配置和监控，系统能够在各种项目环境中稳定运行并发挥最佳效果。新增的CLI目标分解功能增强和验证方法一致性进一步提升了系统的易用性和质量保证，确保每个功能都能得到明确的验证和确认，实现从初始概念到最终部署的完整溯源追踪。