...
## 更新摘要
**变更内容**
- 新增配置参数 `plan_agent_for_decompose` 的详细说明
- 更新内核中配置参数传递的实现细节
- 增强目标分解流程中Plan代理使用的文档说明
- 完善配置系统的架构图和参数传递流程
...
该系统采用分层架构设计，包含目标解析、AI分解、去重处理和持久化存储等多个关键组件，确保生成的功能请求既符合业务目标又避免重复工作。**新增的Plan代理控制参数**使得用户可以灵活控制目标分解过程中是否使用内置的Plan代理，为不同的使用场景提供更好的适应性。
...
B --> |需要改进| D[构建分解提示]
D --> E[检查Plan代理配置]
E --> |plan_agent_for_decompose=true| F[使用Plan代理]
E --> |plan_agent_for_decompose=false| G[直接调用AI代理]
F --> H[调用AI代理]
G --> H
H --> I[解析AI输出]
I --> J{输出格式有效?}
...
**新增的Plan代理控制功能**使得系统能够根据不同场景的需求进行优化：
- 在需要复杂规划和推理时启用Plan代理
- 在追求简单高效时禁用Plan代理
- 通过配置文件实现灵活的参数控制
