...
- [vivify/harness/__init__.py](file://vivify/harness/__init__.py)
- [vivify/harness/doom_loop.py](file://vivify/harness/doom_loop.py)
- [vivify/harness/guides.py](file://vivify/harness/guides.py)
- [vivify/harness/models.py](file://vivify/harness/models.py)
- [vivify/harness/risk_scorer.py](file://vivify/harness/risk_scorer.py)
- [vivify/harness/sensors.py](file://vivify/harness/sensors.py)
- [vivify/knowledge/__init__.py](file://vivify/knowledge/__init__.py)
- [vivify/knowledge/analyzer/static_analyzer.py](file://vivify/knowledge/analyzer/static_analyzer.py)
- [vivify/knowledge/analyzer/semantic_analyzer.py](file://vivify/knowledge/analyzer/semantic_analyzer.py)
- [vivify/knowledge/builder.py](file://vivify/knowledge/builder.py)
- [vivify/knowledge/context_provider.py](file://vivify/knowledge/context_provider.py)
- [vivify/knowledge/maintainer.py](file://vivify/knowledge/maintainer.py)
- [vivify/knowledge/models.py](file://vivify/knowledge/models.py)
- [vivify/knowledge/storage.py](file://vivify/knowledge/storage.py)
</cite>

## 更新摘要
**所做更改**
- 新增Harness系统章节，详细说明PEV验证循环、风险评分、传感器引擎等核心组件
- 新增知识图谱基础设施章节，涵盖构建器、上下文提供者、维护器等模块
- 更新架构概览，从四层演进为五层架构，新增Harness层和知识图谱层
- 扩展配置系统，新增Harness配置和知识图谱配置选项
- 增强AI代理集成，支持知识图谱上下文注入和Harness反馈循环
- 更新依赖关系分析，反映新增的Harness和知识图谱模块
...
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [Harness系统架构](#harness系统架构)
7. [知识图谱基础设施](#知识图谱基础设施)
8. [仪表板系统架构](#仪表板系统架构)
9. [配置加载器增强](#配置加载器增强)
10. [环境变量加载机制增强](#环境变量加载机制增强)
11. [依赖关系分析](#依赖关系分析)
12. [性能考虑](#性能考虑)
13. [故障排除指南](#故障排除指南)
14. [结论](#结论)
...
## 简介
本技术实现参考文档面向架构师与高级开发者，系统性阐述 Vivify 品牌重塑后的全新技术架构与实施细节。项目已重构为包含135个文件的完整生态系统，涵盖kernel系统、AI代理、CLI、修复器、探针、存储、智能分析、**Harness系统**、**知识图谱基础设施**、**仪表板系统**等模块。本文提供精确的改动位置列表与行号对应表，解释技术决策、架构模式与系统约束，涵盖数据流分析、组件交互与集成模式，并给出性能、安全与优化建议，以及故障排除与调试方法。

## 项目结构
本次重构围绕以下核心变更展开：
- 完整的包目录结构：vivify/ 包含 agents、cli、config、daemon、fixers、goals、intelligence、interfaces、kernel、models、pr_mode、probes、reporter、storage、templates、verifier、**harness**、**knowledge**、**dashboard** 等子模块
- 新增135个文件的完整模块体系
...
- 专业化的探针与修复器框架
- **全新的Harness系统：PEV验证循环和风险控制系统**
- **完整的知识图谱基础设施：静态分析、语义增强、增量维护**
...
A --> Q["deployers/ 部署器"]
A --> R["harness/ Harness系统"]
A --> S["knowledge/ 知识图谱"]
R --> R1["doom_loop/ 循环检测"]
R --> R2["guides/ 行为约束"]
R --> R3["risk_scorer/ 风险评分"]
R --> R4["sensors/ 传感器引擎"]
S --> S1["builder/ 图谱构建器"]
S --> S2["context_provider/ 上下文提供者"]
S --> S3["maintainer/ 图谱维护器"]
S --> S4["storage/ 存储管理"]
```
...
- **Harness系统**：PEV验证循环，包含循环检测、行为约束、风险评分和传感器引擎
- **知识图谱系统**：静态分析、语义增强、增量维护的知识图谱基础设施
...
- [vivify/harness/__init__.py:1-18](file://vivify/harness/__init__.py#L1-L18)
- [vivify/knowledge/__init__.py:1-1](file://vivify/knowledge/__init__.py#L1-L1)

## 架构概览
Vivify采用五层架构设计，从底层存储到顶层AI代理形成清晰的职责分离。kernel作为核心编排器，协调各个子系统的协作，实现了"探测-修复-升级"的智能循环。**新增的Harness系统提供PEV验证循环和风险控制**，**知识图谱基础设施提供智能化的项目理解和上下文注入**。**增强的仪表板系统通过RESTful API和WebSocket实现实时数据展示**。**增强的环境变量加载机制确保前台和后台运行模式下环境变量的一致性**。
...
Fixer --> PR["创建PR"]
Agent --> PR
PR --> Harness["Harness验证循环"]
Harness --> Risk{"风险评估"}
Risk --> |低风险| Sensors["运行传感器"]
Risk --> |中高风险| Sensors
Sensors --> Feedback["生成反馈"]
Feedback --> Agent
Agent --> PR
PR --> Monitor["健康监控"]
...
- [vivify/harness/risk_scorer.py:70-134](file://vivify/harness/risk_scorer.py#L70-L134)
- [vivify/harness/sensors.py:31-82](file://vivify/harness/sensors.py#L31-L82)
...
participant Harness as Harness系统
...
Kernel->>Agent : 调用AI代理
Agent-->>Kernel : 返回修复结果
Kernel->>Harness : 触发PEV验证
Harness-->>Kernel : 返回验证结果
Kernel->>PR : 创建PR
end
...
### AI代理集成
QoderCliAgent实现了CodingAgent接口，提供了对Qoder CLI的强大集成。该组件支持并发控制、超时管理、工作空间管理等高级特性，并集成了知识图谱上下文提供者和Harness反馈循环。
...
- **知识图谱集成**：支持精准匹配和历史上下文检索
- **Harness反馈**：接收并处理验证循环反馈
...
**更新** 新增完整的Harness系统，提供PEV验证循环和风险控制系统

Vivify的Harness系统是一个强大的验证和控制框架，基于PEV（Plan-Execute-Verify）循环设计，提供智能的风险评估、行为约束和质量保证机制。系统包含循环检测、行为约束、风险评分和传感器引擎四个核心组件。
...
graph TB
subgraph "Harness系统"
A["HarnessSensorEngine"] --> B["传感器执行器"]
A --> C["风险评分器"]
B --> D["Lint传感器"]
B --> E["TypeCheck传感器"]
B --> F["Test传感器"]
...
**图表来源**
- [vivify/harness/sensors.py:15-207](file://vivify/harness/sensors.py#L15-L207)
- [vivify/harness/risk_scorer.py:47-232](file://vivify/harness/risk_scorer.py#L47-L232)

### 核心组件分析

#### RiskScorer 风险评分器
RiskScorer实现了基于启发式的代码修改风险评估机制，通过分析变更文件、差异统计等信息计算风险分数并确定验证要求。
...
HarnessSensorEngine负责执行项目验证传感器，按照优先级顺序运行LINT、TYPECHECK、TEST、BUILD等传感器，收集结果并生成反馈。

**执行策略**：
- 传感器优先级：LINT → TYPECHECK → TEST → BUILD
- 超时保护：每个传感器有独立的超时配置
- 增量检测：LINT支持针对变更文件的增量运行
- 输出截断：超过2000字符的输出进行头尾截断

**章节来源**
- [vivify/harness/sensors.py:15-207](file://vivify/harness/sensors.py#L15-L207)

#### DoomLoopDetector 循环检测器
DoomLoopDetector检测AI代理是否陷入无效的重复操作模式，通过滑动窗口分析操作指纹来识别循环行为。

**检测机制**：
- 操作指纹：category + issue_hash + action_type
- 滑动窗口：最近10次操作
- 重复阈值：>=3次重复触发循环检测
- 逃逸策略：跳过当前问题、延长冷却时间、标记人工审核

**章节来源**
- [vivify/harness/doom_loop.py:11-136](file://vivify/harness/doom_loop.py#L11-L136)

#### GuidesManager 行为约束管理器
GuidesManager管理项目特定的AI行为约束指南，从.md文件加载并注入到AI代理的提示词中。
...
- develop_*.md：开发功能时使用
- verify_*.md：验证功能时使用
- evaluate_*.md：评估功能时使用

**章节来源**
- [vivify/harness/guides.py:33-255](file://vivify/harness/guides.py#L33-L255)
...
#### HarnessReport 验证报告模型
包含传感器结果、整体通过状态、风险等级、反馈提示和循环检测状态。

#### SensorResult 传感器结果模型
记录单个传感器的执行结果，包括类型、通过状态、输出、持续时间和退出码。

**章节来源**
- [vivify/harness/models.py:7-39](file://vivify/harness/models.py#L7-L39)

## 知识图谱基础设施

**更新** 新增完整的知识图谱基础设施，提供智能化的项目理解和上下文注入

Vivify的知识图谱系统是一个完整的项目理解框架，通过静态分析、语义增强和增量维护构建和维护项目知识图谱，为AI代理提供智能化的上下文信息。
...
graph TB
subgraph "知识图谱系统"
A["KnowledgeBuilder"] --> B["StaticAnalyzer"]
...
G --> I
H --> I
I --> J["KnowledgeStorage"]
J --> K["graph.json"]
J --> L["meta.json"]
J --> M["conventions.json"]
J --> N["modules/"]
I --> O["KnowledgeContextProvider"]
O --> P["精准匹配"]
O --> Q["相关模块"]
O --> R["历史经验"]
end
...
**图表来源**
- [vivify/knowledge/builder.py:53-780](file://vivify/knowledge/builder.py#L53-L780)
- [vivify/knowledge/context_provider.py:29-644](file://vivify/knowledge/context_provider.py#L29-L644)

### 核心组件分析

#### KnowledgeBuilder 知识图谱构建器
KnowledgeBuilder实现了三阶段的知识图谱构建流程：结构分析、语义增强和约定提取，支持全量重建和增量更新模式。

**构建流程**：
1. **结构分析阶段**：静态分析生成结构图
2. **语义增强阶段**：LLM/wiki增强模块描述、责任和层级
3. **约定提取阶段**：从代码样本中提取命名规范、导入风格等
...
#### KnowledgeContextProvider 上下文提供者
KnowledgeContextProvider为AI代理提供智能的上下文注入，采用三层注入策略：L1（总是）、L2（相关性）、L3（按需）。

**注入策略**：
- **L1层（约500tokens）**：项目概览 + 模块列表（始终包含）
- **L2层（约1500tokens）**：Top-3相关模块的详细信息
- **L3层（可选）**：特定API签名的按需注入

**匹配算法**：
- 实体提取：从功能标题和描述中提取函数名、类名、文件名
- 精准匹配：在知识图谱中搜索匹配节点
- 相关性评分：基于Jaccard相似度计算模块相关性
- 边缘关系：展示节点间的依赖和调用关系

**章节来源**
- [vivify/knowledge/context_provider.py:29-644](file://vivify/knowledge/context_provider.py#L29-L644)

#### KnowledgeMaintainer 知识图谱维护器
KnowledgeMaintainer负责在守护进程运行期间保持知识图谱的新鲜度，定期检查代码变更并触发增量更新。
...
**章节来源**
- [vivify/knowledge/maintainer.py:19-74](file://vivify/knowledge/maintainer.py#L19-L74)

### 数据模型

#### KnowledgeGraph 知识图谱模型
完整的知识图谱数据结构，包含元数据、节点、边、架构层级和代码约定。
...
**章节来源**
- [vivify/knowledge/models.py:8-268](file://vivify/knowledge/models.py#L8-L268)

#### KnowledgeStorage 存储管理器
管理知识图谱文件的读写操作，采用标准化的存储布局。

**存储布局**：
- `.vivify/knowledge/graph.json`：完整图数据
- `.vivify/knowledge/meta.json`：元数据
- `.vivify/knowledge/conventions.json`：代码约定
- `.vivify/knowledge/modules/`：按模块分割的详细信息

**章节来源**
- [vivify/knowledge/storage.py:14-134](file://vivify/knowledge/storage.py#L14-L134)
...
- **Harness配置**：风险评分、传感器命令、反馈超时等
- **知识图谱配置**：Qoder CLI二进制、Wiki路径、权限模式等
...
## 依赖关系分析
Vivify采用了清晰的五层依赖结构，每个模块都有明确的职责边界和依赖关系。
...
N["dashboard/"] --> D
N --> O["FastAPI"]
P["deployers/"] --> Q["subprocess"]
end
subgraph "Harness层"
R["harness/"] --> S["doom_loop/"]
R --> T["guides/"]
R --> U["risk_scorer/"]
R --> V["sensors/"]
R --> W["models/"]
...
subgraph "知识图谱层"
X["knowledge/"] --> Y["builder/"]
X --> Z["context_provider/"]
X --> AA["maintainer/"]
X --> BB["storage/"]
CC["agents/"] --> DD["knowledge/"]
EE["intelligence/"] --> DD
FF["kernel/"] --> DD
end
```
...
- **知识图谱优化**：增量更新机制减少全量重建开销
- **Harness性能**：传感器执行优先级排序，快速失败减少等待时间
- **风险评分缓存**：避免重复计算相同文件的风险评分
...
- **Harness传感器失败**：检查传感器命令配置，验证工具可用性
- **知识图谱构建失败**：检查Qoder CLI可用性，验证项目结构
- **循环检测误报**：调整循环检测阈值和窗口大小
...
- [vivify/cli/run_cmd.py:36-74](file://vivify/cli/run_cmd.py#L36-L74)
- [vivify/harness/sensors.py:84-131](file://vivify/harness/sensors.py#L84-L131)
- [vivify/knowledge/builder.py:92-110](file://vivify/knowledge/builder.py#L92-L110)
...
**新增的Harness系统进一步增强了系统的质量保证能力**，通过PEV验证循环、风险评估和行为约束机制，确保AI生成的代码变更符合项目规范和质量标准。**完整的知识图谱基础设施提供了智能化的项目理解和上下文注入能力**，显著提升了AI代理的决策质量和开发效率。**全新的五层架构设计（核心层、支持层、Harness层、知识图谱层、仪表板层）体现了系统的成熟度和扩展性**。**增强的配置加载器支持环境变量覆盖，提高了配置的灵活性和部署便利性**。**最关键的环境变量加载机制增强解决了前台运行模式下环境变量加载不一致的问题，确保了系统在不同运行模式下的一致性和可靠性**。

新的架构不仅提升了系统的可维护性和扩展性，还为未来的功能增强奠定了坚实基础。Harness系统、知识图谱系统、仪表板系统、配置增强、环境变量机制和核心功能的完美结合，使得Vivify成为了一个真正意义上的智能项目管理系统，代表了软件自动化领域的先进水平。
...
- **harness**：Harness系统配置，包括风险评分、传感器命令、反馈超时
- **knowledge**：知识图谱配置，包括Qoder CLI二进制、Wiki路径、权限模式
...
- `VIVIFY__harness__risk_scoring_enabled=true`
- `VIVIFY__knowledge__qodercli_binary=qodercli`
...
- [vivify/cli/run_cmd.py:36-74](file://vivify/cli/run_cmd.py#L36-L74)
- [vivify/daemon/manager.py:62-93](file://vivify/daemon/manager.py#L62-L93)
- [vivify/harness/risk_scorer.py:67-89](file://vivify/harness/risk_scorer.py#L67-L89)
- [vivify/knowledge/builder.py:56-74](file://vivify/knowledge/builder.py#L56-L74)