...
``qodercli`` (see ``auto_heal.agents.qodercli_agent``), but the interface is
...
class CodingAgent(ABC):
    ...
    def heal(
        ...
        * Tag any spawned subprocesses with ``AUTO_HEAL_AGENT=1`` in their
    ...
    def healthcheck(self) -> tuple[bool, str]:
        """Quick ``--version`` style probe used by ``auto-heal doctor``.
