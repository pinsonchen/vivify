...
``auto_heal.goals.decomposer`` delegates the heavy lifting to a ``CodingAgent``
...
from auto_heal.models import FeatureRequest, FeatureSpec, Goal, KpiSnapshot
...
class GoalDecomposer(ABC):
    ...
    def parse_goals(self, md_text: str) -> List[Goal]:
        ...
        input rather than silently dropping goals — ``auto-heal goals show``
