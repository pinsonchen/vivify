...
This is the default storage backend wired up by ``auto-heal init``. It keeps
all state inside a single ``.auto-heal/state.db`` file using WAL mode for
concurrent reads. Schema lives under ``auto_heal/storage/migrations/`` and is
...
class SqliteStorageProvider(StorageProvider):
    ...
    def _run_migrations(self) -> None:
        ...
        migration_files = sorted(
            f
            for f in resources.files("auto_heal.storage.migrations").iterdir()
