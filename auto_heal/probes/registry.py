...
1. **Built-in YAML** under ``auto_heal/probes/builtin/*.yml`` (shipped wheel).
2. **User YAML** under ``<repo>/.auto-heal/probes/*.yml``.
3. **User Python** under ``<repo>/.auto-heal/probes/*.py`` — each module is
...
``.auto-heal.yml`` with everything the registry could find.
"""
...
from auto_heal.interfaces.probe import Probe
...
class ProbeRegistry:
    ...
    def load_builtins(self) -> int:
        """Load every ``*.yml`` shipped under ``auto_heal/probes/builtin``."""
        count = 0
        try:
            pkg = resources.files("auto_heal.probes.builtin")
    ...
    def load_user_dir(self, user_dir: Path | str | None) -> int:
        """Load YAML + Python probes from a user directory (typically ``.auto-heal/probes``)."""
        if not user_dir:
            return 0
        path = Path(user_dir)
        if not path.is_dir():
    ...
    def _load_python_plugin(self, path: Path) -> int:
        spec = importlib.util.spec_from_file_location(f"auto_heal_userprobe_{path.stem}", path)
