...
This is a near-verbatim port of ``channels-monitor/auto_heal/healer.py::heal``,
...
* Subprocess environment is tagged with ``AUTO_HEAL_AGENT=1`` so
  :mod:`auto_heal.agents.slot_manager` can reliably count concurrent runs.
...
from auto_heal.agents.slot_manager import AGENT_ENV_TAG, wait_for_slot
from auto_heal.interfaces.agent import CodingAgent
from auto_heal.models import AgentResult
...
class QoderCliConfig:
    """Configuration for :class:`QoderCliAgent` (mirrors ``.auto-heal.yml::agent.qodercli``)."""
