...
1. Run the enabled probes via :func:`auto_heal.probes.runner.run_probes`.
2. For every Issue:
...
5. Detect changes to auto-heal's own code via :func:`compute_code_hash`; if
...
from auto_heal.agents.prompts import builders, parsers
from auto_heal.fixers.registry import FixerRegistry
from auto_heal.interfaces.agent import CodingAgent
from auto_heal.interfaces.fixer import FixContext, FixResult
...
from auto_heal.kernel.escalator import Escalator, EscalationPolicy
from auto_heal.kernel.failure_tracker import FailureTracker
from auto_heal.kernel.feature_pipeline import FeaturePipeline, FeatureRunReport
from auto_heal.kernel.health_monitor import HealthMonitor
...
from auto_heal.pr_mode.auto_merge import AutoMerge
from auto_heal.pr_mode.pr_creator import PrCreator
from auto_heal.pr_mode.quality_check import run_quality_checks
from auto_heal.pr_mode.self_grow_guard import classify_worktree
from auto_heal.pr_mode.worktree import WorktreeManager
from auto_heal.probes.runner import aggregate_issues, run_probes
...
class Kernel:
    """Drives the entire auto-heal loop. One per process."""
    ...
    def run_forever(self, *, max_rounds: Optional[int] = None) -> None:
        ...
        while True:
            ...
            if self._code_hash_changed(report.code_hash):
                logger.warning(
                    "auto-heal package hash changed (%s → %s); requesting restart",
    ...
    def _try_agent_fix(self, issue: Issue, *, report: RoundReport) -> bool:
        ...
        try:
            ...
            pr = self.deps.pr_creator.push_and_open(
                wt,
                title=f"auto-heal: {issue.title}"[:200],
