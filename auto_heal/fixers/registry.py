...
from auto_heal.interfaces.fixer import Fixer
...
class FixerRegistry:
    ...
    def load_builtins(self) -> int:
        """Import every module under ``auto_heal.fixers.builtin`` and collect
        instances assigned to ``FIXER`` / ``FIXERS``."""
        import auto_heal.fixers.builtin as pkg
        count = 0
    ...
    def load_user_dir(self, user_dir: Path | str | None) -> int:
        ...
        for entry in sorted(path.iterdir()):
            ...
            spec = importlib.util.spec_from_file_location(
                f"auto_heal_userfixer_{entry.stem}", entry
            )
