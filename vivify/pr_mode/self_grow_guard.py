"""Self-growth guard — classifies diffs touching vivify's own code.

When the agent decides to modify *vivify itself* (a "self-grow" PR), this
module decides:
...
* **kernel** PR — kernel / interfaces / storage code changed. Forced to draft,
  labelled ``vivify:kernel-change``, and **never** auto-merged.
* **mixed** PR — both. Treated as kernel for safety.
* **external** — change does not touch the vivify package. No special
  handling.
...
# Anything inside the vivify package not covered by PLUGIN_PATHS counts as
# a "kernel" change for guarding purposes.
KERNEL_PREFIX = "vivify/"
...
class GuardDecision:
    ...
    def labels(self) -> list[str]:
        if self.classification is DiffClass.KERNEL:
            return ["vivify:kernel-change"]
        if self.classification is DiffClass.MIXED:
            return ["vivify:kernel-change", "vivify:plugin-change"]
        if self.classification is DiffClass.PLUGIN:
            return ["vivify:plugin-change"]
        return []
