...
default branch under ``<repo>/.vivify/worktrees/<slug>/``. The worktree
...
class WorktreeManager:
    """Create, list, and remove vivify worktrees for a repo."""
    ...
    def __init__(
        ...
        *,
        ...
        self.worktree_base = (
            Path(worktree_base)
            if worktree_base
            else self.repo_root / ".vivify" / "worktrees"
        )
    ...
    def list(self) -> list[Worktree]:
        """Return vivify worktrees currently registered with git."""
        result = _run(
