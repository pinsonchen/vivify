"""``vivify probes`` — list / test / enable / disable."""
...
def register(sub: argparse._SubParsersAction) -> None:
    ...
    en = sp.add_parser("enable", help="Add a probe to the enabled list (.vivify.yml).")
...
def run_test(args: argparse.Namespace) -> int:
    ...
    ctx = ProbeContext(
        ...
        config=cfg, storage=storage, logger=logging.getLogger("vivify.cli.probes"),
    )
...
def _rewrite_enabled(args, *, transform) -> int:
    """Mutate cfg.probes.enabled and write back to .vivify.yml."""
    try:
        import yaml  # type: ignore[import-untyped]
    except ImportError:
        print("PyYAML required to edit .vivify.yml")
        return 2
    path = Path(getattr(args, "config", None) or ".vivify.yml")
    raw = {}
    if path.exists():
