...
GIT_PR_SNIPPET = dedent(
    ...
    vivify is in PR-only mode. **Never push directly to the base branch.**
    Instead:

    1. Stage and commit your edits inside the current worktree:
       ```bash
       git add -A
       git commit -m "vivify: <one-line summary>"
       ```
    2. Push the branch (vivify already created it for you):
       ```bash
       git push -u origin "$(git rev-parse --abbrev-ref HEAD)"
       ```
    3. The vivify kernel will open the Pull Request via the GitHub CLI
...
SELF_IMPROVE_SNIPPET = dedent(
    ...
    You may modify the vivify package itself to make future runs smarter,
    *within these whitelisted paths only*:

    - `vivify/probes/builtin/**` — tweak detection rules / thresholds
    - `vivify/fixers/builtin/**` — add or improve deterministic fixes
    - `vivify/agents/prompts/templates/**` — improve prompt phrasing
    - `vivify/agents/prompts/snippets.py` — improve reusable snippets
