...
spawned ``qodercli`` is tagged with the env variable ``VIVIFY_AGENT=1`` so
...
AGENT_ENV_TAG = "VIVIFY_AGENT"


def count_agent_processes(binary_basename: str = "qodercli") -> int:
    """Return the number of running agent processes started by vivify.
...
def _proc_has_env_tag(pid: int) -> bool:
    """True if the process was spawned with ``VIVIFY_AGENT=1``."""
    # Linux fast path
